...
//    @Override
//    public void generate(int id, int width, int height, String filepath, int col) throws IOException, DocumentException {
...
public void generate(int id, int width, int height, String filepath, int col) {
    ...
    for (Map.Entry<String, String> entry : txt.entrySet()) {
        BitMatrix bitMatrix;
        try {
            PdfReader reader = new PdfReader("/home/vinayasree/Downloads/Sticker-PDF.pdf");
            ...
            float y = (pageSize.getBottom() + pageSize.getTop()) / 2 - qrImage.getHeight()/2 -1; // adjust the vertical position as needed
            contentByte.beginText();
            contentByte.setFontAndSize(BaseFont.createFont(), 14); // set the font and font size
            contentByte.showTextAligned(Element.ALIGN_CENTER, text, x, y, 0); // align the text to the center of the page
            contentByte.endText();
